...
class _PlanConfigPageState extends State<PlanConfigPage> with SingleTickerProviderStateMixin {
  ...
  Map<String, dynamic>? _getSelectedPlan() {
    final plan = widget.order.selectedPlan;
    if (plan != null) {
      return {
        'capacity': plan.capacity,
        'label': plan.label,
        'price': plan.price,
        'plans_id': plan.plansId,
      };
    }
    return null;
  }
  ...
  Map<String, dynamic>? _getSelectedDuration() {
    final duration = widget.order.selectedDuration;
    if (duration != null) {
      return {
        'duration_id': duration.durationId,
        'duration_time_limit': duration.durationTimeLimit,
        'gst': duration.gst,
        'discount': duration.discount,
        'security_deposit': duration.securityDeposit,
      };
    }
    return null;
  }
  ...
  String _getModelName() {
    final modelName = widget.order.modelName;
  ...
  String _getDeviceId() {
    final deviceId = widget.order.wpDeviceId.isEmpty ? widget.macId : widget.order.wpDeviceId;
    if (deviceId.isEmpty) {
      return 'N/A';
    }
    return deviceId;
  ...
  Future<void> _sendPlanConfig() async {
    ...
    try {
      ...
      if (ackSuccess) {
        final updatedOrderJson = widget.order.toJson();
        updatedOrderJson['isSetup'] = true;
        updatedOrderJson['plan_config'] = planConfig;
        final Order updatedOrder = Order.fromJson(updatedOrderJson);

        final List<Order> updatedOrders = controller.orders.map((order) {
        ...
        controller.orders.assignAll(updatedOrders);

        if (controller.activeSubscription.value?.id == widget.order.id) {
          controller.activeSubscription.value = updatedOrder;
        }
        ...
        if (mounted) {
          Get.offAll(() => const AppUserLandingPage());
        }
      }
    } catch (e) {
